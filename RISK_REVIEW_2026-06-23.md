...
| E2 | 部分修 | 低樣本已降風險；「最近一日符合」已改成「近期出現」並加不代表後續走勢旁註。常態區間呈現仍可再打磨。 |
| F1 | 已修 | 雷達/Excel/本地 UI 的「現價」已正名為「最近收盤」。 |
| F2 | 部分修 | 雷達 payload 已新增 `latest_close`，舊快照會 backfill；`current_price` 仍保留為相容 alias，後續可逐步淘汰。 |
...
目前下一步建議：
1. 先延伸 `B3`：接官方 CSV/多年度交易日曆，取代目前的 2026 靜態休市日。
2. 再補 `A3/F2/G1/E2`：分別處理歷史股票股利資料源、payload 命名債、兩個更新流程說明、回測卡片措辭。
3. 視情況處理 `D3/B5`：D3 是效能成本，B5 等交易日曆完整後再重調門檻。
