...
| B2 | 部分修 | 週末 target 已退到最近工作日；國定假日尚未接入交易日曆。 |
| B3 | 部分修 | 已接入 2026 TWSE 休市日，缺口天數與 target date 會跳過 2026 國定假日；仍待官方 CSV/多年度交易日曆。 |
| D1 | 已緩解 | 法人 T86 連續空日閥門已放寬到 20 個交易日；更完整做法仍是用交易日清單驅動。 |
| E1 | 已修 | 低樣本歷史頻率不再顯示常態 100%/0% 面積。 |
| E2 | 部分修 | 低樣本已降風險；「最近一日符合」和常態區間的卡片措辭仍可再保守。 |
| F1 | 已修 | 雷達/Excel/本地 UI 的「現價」已正名為「最近收盤」。 |
| F2 | 未修 | 內部 payload 仍保留 `current_price = close` 相容欄位，後續可漸進新增 `latest_close` 並淘汰。 |
| G1 | 部分修 | 全市場資料下載已搬到「本地資料」，雷達中心改成快照/排行；兩個更新流程的資料關係仍可再做 UI 文案或狀態整合。 |
| A3 | 未修 | TWT49U 歷史除權息目前仍只可靠補「現金除息」；歷史股票股利需先確認官方欄位語意，不能把權值誤當每股股票股利。 |
| D2 | 未修 | 日線已最新時仍會早退，可能跳過 profile/估值/月營收/財報更新。 |
| D3 | 暫緩 | 日線按月重抓是效能成本，不是資料正確性錯誤。 |
| B5 | 暫緩 | SUSPECT 門檻待交易日曆導入後再重調較合理。 |

目前下一步建議：
1. 先補 `D2`：把「價格日線同步」與「profile/估值/月營收/財報 metadata refresh」拆開，避免價格 current 時其他資料永遠不更新。
2. 再延伸 `B3`：接官方 CSV/多年度交易日曆，取代目前的 2026 靜態休市日。
3. 再補 `A3/F2/G1/E2`：分別處理歷史股票股利資料源、payload 命名債、兩個更新流程說明、回測卡片措辭。
...
- **B5**（SUSPECT 誤標）：等假日表(B3)做了多半自然改善。
