...
| B3 | 未修 | business-day 計數仍只跳六日，還沒有台股國定假日/補班交易日資料源。 |
...
目前下一步建議：
1. 先補 `B3`：建立可注入的台股交易日/休市日邏輯，讓缺口天數和 target date 不把國定假日算成缺漏。
2. 再補 `D2`：把「價格日線同步」與「profile/估值/月營收/財報 metadata refresh」拆開，避免價格 current 時其他資料永遠不更新。
