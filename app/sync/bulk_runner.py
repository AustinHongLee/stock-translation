...
def build_bulk_plan(db_path, *, lookback_days: int = 365, request_interval: float = 0.2) -> BulkPlan:
    ctx: dict = {}
    today = date.today()
    ...
    def skip(sid: str) -> bool:
        ...
        if not latest:
            return False
        return (today - latest[-1].date).days <= 3

    return BulkPlan(list_stocks=list_stocks, sync_one=sync_one, prelude=prelude, skip=skip)
