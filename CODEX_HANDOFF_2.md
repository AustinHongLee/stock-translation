...
- **驗收**：關掉重開能續傳（可驗）；只重試失敗可運作；local-data 大量資料下順；harness 過。
  ...
  - 2026-06-23 驗證（TWSE 官方多年度交易日曆）：`app/analyze/twse_calendar.py` 改成內建 TWSE 官方開休市資料 2024–2026，並新增可解析官方 RWD JSON / OpenAPI CSV row 的 parser；`SQLiteStore.compute_data_coverage()` 的 `hole_count` 改用同一份台股交易日計數，不再只排週末。`RISK_REVIEW_2026-06-23.md` 將 B3 更新為已修（未來年度待 TWSE 公布後擴充）。已跑 `python -m unittest tests.test_twse_calendar tests.test_data_gap tests.test_market_calendar tests.test_sqlite_store tests.test_sync_service tests.test_web_api`（39 OK）、`python -m py_compile app/analyze/twse_calendar.py app/analyze/data_gap.py app/analyze/market_calendar.py app/store/sqlite_store.py app/sync/service.py`。
  ...
  - 2026-06-23 驗證（淘汰雷達 current_price alias）：新產生的雷達 payload 不再寫 `current_price`，前端雷達卡片改讀 `latest_close`；`load_value_screener()` 仍會把舊快照的 `current_price` backfill 成 `latest_close`，Excel 也保留舊檔 fallback。`RISK_REVIEW_2026-06-23.md` 將 F2 更新為已修。已跑 `python -m unittest tests.test_value_screener tests.test_excel_screener_export tests.test_web_api tests.test_ui_theme`（29 OK）、`node --check app/ui/static/app.js`、`python -m py_compile app/screener/value.py app/exporters/excel.py`。
  ...
  - 2026-06-23 驗證（Opus 第二/三輪測試回饋修正）：`RISK_REVIEW_2026-06-23.md` 新增第二輪/第三輪審查後，已修 Z1/H1/H2/S1/SR-1/MA-1，並緩解 MA-2：雷達、單檔同步、全市場下載共用 6 年 TWT49U 歷史股利回補；bulk 法人 T86 改用 TWSE 交易日曆；bulk running/paused 時同步/更新雷達回 409；`TwseClient` 對全市場 metadata 與 TWT49U 年度檔做 15 分鐘共享快取；支撐/壓力突破/破底時錯邊關卡回 `None` 並標新高/新低；個股 payload 新增 `ma_prices` 供圖表均線暖身，畫面仍只顯示可視段。已跑 `python -m unittest discover -s tests`（251 OK）、`node --check app/ui/static/app.js`、`node --check app/ui/static/sw.js`、`python -m compileall app tests`、`git diff --check`、行首衝突標記/`debugger`/`console.log`/紅線掃描皆無命中。
  - 2026-06-23 驗證（支撐/壓力與 local-data 收尾）：`compute_support_resistance()` 末端轉折在至少 1 根右側 K 線確認後可先納入；樞紐關卡需鄰近觸及或收盤貼近確認，單一長影線不再直接成壓力；狀態新增「支撐上方 / 壓力下方 / 關卡待確認」，避免找不到完整上下關卡時仍顯示「區間中」。`/api/local-data` 盤點 coverage 改優先讀 `data_coverage` 索引，bulk 日線寫入後會刷新 daily coverage，降低盤點頁 O(N) 掃表成本；SR 仍即時計算，後續若資料量再放大可再落表。`RISK_REVIEW_2026-06-23.md` 已更新 S2/SR-2/SR-3/SR-4 狀態。已跑 `python -m unittest discover -s tests`（254 OK）、`node --check app/ui/static/app.js`、`node --check app/ui/static/sw.js`、`python -m compileall app tests`、`git diff --check`、行首衝突標記/`debugger`/`console.log`/紅線掃描皆無命中。
